...
auth_handler.post(async (req: NextApiRequest, res: NextApiResponse) => {
    ...
    if (!agreed) {
        return res.status(400).send({ message: 'All applicants must agree to our terms and conditions' })
    }

    if (store_exists) {
        return res.status(422).send({ message: 'One account can only have one store' })
    }
    if (name_exists) {
        return res.status(400).send({ message: 'Name already in use. If the name belongs to you contact our support team' })
    }
    if (!company_name) {
